...
int main( int argc, char * argv[] )
{
	...
	while ( cin >> px >> py >> pz >> E )
	{
		...
		phisIn.push_back( dummy1.Phi() );
		rapiditiesIn.push_back( dummy1.Rapidity() );
		ptsIn.push_back( dummy1.Pt() );
		energiesIn.push_back( E );
		etasIn.push_back( dummy1.Eta() );
	}

	//Copy input data into flat arrays
	unsigned int const totalObjects = phisIn.size();
	double phis[ totalObjects ];
	copy( phisIn.begin(), phisIn.end(), phis );
	double rapidities[ totalObjects ];
	copy( rapiditiesIn.begin(), rapiditiesIn.end(), rapidities );
	double pts[ totalObjects ];
	copy( ptsIn.begin(), ptsIn.end(), pts );
	double energies[ totalObjects ];
	copy( energiesIn.begin(), energiesIn.end(), energies );
	double etas[ totalObjects ];
	copy( etasIn.begin(), etasIn.end(), etas );
	double cachedMinKts[ totalObjects ];
	fill_n( cachedMinKts, totalObjects, 0.0 );
	unsigned int cachedMinKtPairs[ totalObjects ];
	fill_n( cachedMinKtPairs, totalObjects, 0 );
	bool wasDeleted[ totalObjects ];
	fill_n( wasDeleted, totalObjects, false );
